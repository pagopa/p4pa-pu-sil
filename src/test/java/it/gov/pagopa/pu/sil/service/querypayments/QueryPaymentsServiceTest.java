...
class QueryPaymentsServiceTest {
  ...
  @InjectMocks
  private QueryPaymentsService service;
  ...
  void testProcessRequestOk(QueryPaymentStatusType idType) {
    ...
    String idParam = switch (idType) {
      case INSTALLMENT_ID -> installment.getInstallmentId().toString();
      case IUD -> installment.getIud();
      case NOTICE_NUMBER -> installment.getIuv();
    };
    ReceiptWithAdditionalNodeDataDTO receiptDTO = podamFactory.manufacturePojo(ReceiptWithAdditionalNodeDataDTO.class);
    request = new PaymentStatusRequest(org.getIpaCode(), idType, idParam, true);
    ...
    when(debtPositionInstallmentFacadeServiceMock.fetch(request, org, accessToken))
      .thenReturn(pairList);
    ...
    // When
    PaymentStatusResponseDTO result = service.processRequest(request, userInfo, accessToken);
    // Then
  ...
  void testProcessRequestUnpaidInstallment() {
    ...
    request = new PaymentStatusRequest(org.getIpaCode(), INSTALLMENT_ID, unpaidInstallment.getInstallmentId().toString(), true);
    PaymentStatusResponseDTO expectedResponse = new PaymentStatusResponseDTO()
    ...
    when(organizationServiceMock.getOrganizationById(org.getOrganizationId(), accessToken)).thenReturn(Optional.of(org));
    when(debtPositionInstallmentFacadeServiceMock.fetch(request, org, accessToken)).thenReturn(pairList);
    // When
    PaymentStatusResponseDTO result = service.processRequest(request, userInfo, accessToken);
    // Then
    assertEquals(expectedResponse, result);
  ...
  void testProcessRequestExpiredInstallment() {
    ...
    request = new PaymentStatusRequest(org.getIpaCode(), INSTALLMENT_ID, expiredInstallment.getInstallmentId().toString(), true);
    PaymentStatusResponseDTO expectedResponse = new PaymentStatusResponseDTO()
  ...
  void testProcessRequestUnpayableInstallment() {
    ...
    request = new PaymentStatusRequest(org.getIpaCode(), INSTALLMENT_ID, unpayableInstallment.getInstallmentId().toString(), true);
    PaymentStatusResponseDTO expectedResponse = new PaymentStatusResponseDTO()
