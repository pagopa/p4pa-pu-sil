package it.gov.pagopa.pu.sil.service.inbound.payments.singleimport.soap;
...
class PaaSILImportaDovutoServiceTest {
  ...
  void givenValidDataWhenPaaSILImportaDovutoThenOk(String action) {
    ...
    when(paaSILImportaDovutoMapperMock.mapRequestToDebtPosition(request, org, TOKEN)).thenReturn(Pair.of(debtPositionDTO, action));

    switch (action) {
      case Constants.LEGACY_IMPORT_ACTION_INSERT:
        when(manageDebtPositionServiceMock.createDebtPositions(List.of(debtPositionDTO), TOKEN)).thenReturn(List.of(processedDebtPositionDTO));
        break;
      case Constants.LEGACY_IMPORT_ACTION_MODIFY, Constants.LEGACY_IMPORT_ACTION_CANCEL:
        when(debtPositionServiceMock.getDebtPositionsByOrganizationIdAndIud(eq(orgId), eq(iud), Mockito.any(), eq(TOKEN)))
        ...
        when(manageDebtPositionServiceMock.manageDebtPositionInstallments(processedDebtPositionDTO.getDebtPositionId(), manageDebtPositionDTO, TOKEN))
          .thenReturn(processedDebtPositionDTO);
        break;
      case Constants.LEGACY_IMPORT_ACTION_PRINT:
        when(debtPositionServiceMock.getDebtPositionsByOrganizationIdAndIud(eq(orgId), eq(iud), Mockito.any(), eq(TOKEN)))
          .thenReturn(List.of(processedDebtPositionDTO));
        when(noticeServiceMock.generateNotice(processedInstallmentDTO.getNav(), processedDebtPositionDTO, TOKEN))
  ...
  void givenInvalidDataWhenPaaSILImportaDovutoThenException(String testCase) {
    ...
    switch (testCase) {
      case "invalidAction":
        when(paaSILImportaDovutoMapperMock.mapRequestToDebtPosition(request, org, TOKEN)).thenReturn(Pair.of(debtPositionDTO, "INVALID"));
        break;
